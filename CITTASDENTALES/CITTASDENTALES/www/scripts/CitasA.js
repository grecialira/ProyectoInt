$(document).ready(function () {
    ...
    // Evento para el boton de la clase .eliminar
     $('#TablaMensajes').on('click', '.eliminar', function () {
        var id = $(this).data('id');    // recupera el Id del alumno
        // Cuadro de dialogo para preguntar si se va a eliminar
        bootbox.confirm("¿Eliminar Mensaje?", function (res) {
            if (res == true)
                eliminarMensaje(id);
        });
    });
    $('#Busca').validate({
        ...
        messages: {
            Buscar: { required: "Escriba un nombre para buscar." },
...
function leerBusqueda() {
    ...
    var dateH;
    var resultH;
    var metodo = "http://ticutt.mx/mtic5c/clinicajoya/Citas_Services.asmx/GetAlumnosP";
...
function eliminarAlumno(ida) {
    $.ajax({
        type: 'POST',
        url: 'http://ticutt.mx/mtic5c/clinicajoya/Citas/EliminarAlumnoA',
        ...
        success: function (data) {
            var res = parseInt(data.d);
            if (res == 1) {
                bootbox.alert('No se Canceló la Cita');
                leerAlumnos();
            }
            else
...
function ResponderMensaje(ida) {
    ...
    $.ajax({
        type: 'POST',
        url: "http://ticutt.mx/mtic5c/clinicajoya/Mensajes/Responder" ,
        data: JSON.stringify({ id: ida ,alumno: alu }),
        dataType: 'json',
        contentType: 'application/json',
        error: function (xhr) {
            bootbox.alert('Ocurrio un error en la petición');
        ...
        success: function (data) {
            ...
            if (res == 1) {
                ...
                window.form2.reset(); // LImpia los campos del formulario
